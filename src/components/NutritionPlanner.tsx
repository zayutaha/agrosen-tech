...
const NutritionPlanner = () => {
  ...
  const getNPKRecommendation = (nutrient: string, value: number) => {
    ...
    if (value < nutrientData.low.threshold)
      return { ...nutrientData.low, status: "low" };
    if (value > nutrientData.high.threshold)
      return { ...nutrientData.high, status: "high" };
  ...
  const phosphorusRec = getNPKRecommendation(
    "phosphorus",
    latestReading.phosphorus,
  );
  const potassiumRec = getNPKRecommendation(
    "potassium",
    ...
    <div className="space-y-6">
      ...
      <Card
        className={`border-l-4 ${
          nitrogenRec.status === "low"
            ? "border-l-destructive"
            : nitrogenRec.status === "high"
              ? "border-l-destructive"
              : "border-l-secondary"
        }`}
      >
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant={
                nitrogenRec.status === "optimal" ? "secondary" : "destructive"
              }
            >
              {nitrogenRec.status}
            </Badge>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="bg-accent/50 p-4 rounded-lg space-y-2">
            <div>
              <strong>Action:</strong> {nitrogenRec.action}
            </div>
            <div>
              <strong>Dosage:</strong> {nitrogenRec.dosage}
            </div>
            <div>
              <strong>Timing:</strong> {nitrogenRec.timing}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card
        className={`border-l-4 ${
          phosphorusRec.status === "low"
            ? "border-l-destructive"
            : phosphorusRec.status === "high"
              ? "border-l-destructive"
              : "border-l-secondary"
        }`}
      >
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant={
                phosphorusRec.status === "optimal" ? "secondary" : "destructive"
              }
            >
        ...
        <CardContent className="space-y-3">
          ...
          <div className="bg-accent/50 p-4 rounded-lg space-y-2">
            <div>
              <strong>Action:</strong> {phosphorusRec.action}
            </div>
            <div>
              <strong>Dosage:</strong> {phosphorusRec.dosage}
            </div>
            <div>
              <strong>Timing:</strong> {phosphorusRec.timing}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card
        className={`border-l-4 ${
          potassiumRec.status === "low"
            ? "border-l-destructive"
            : potassiumRec.status === "high"
              ? "border-l-destructive"
              : "border-l-secondary"
        }`}
      >
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant={
                potassiumRec.status === "optimal" ? "secondary" : "destructive"
              }
            >
              {potassiumRec.status}
            </Badge>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="bg-accent/50 p-4 rounded-lg space-y-2">
            <div>
              <strong>Action:</strong> {potassiumRec.action}
            </div>
            <div>
              <strong>Dosage:</strong> {potassiumRec.dosage}
            </div>
            <div>
              <strong>Timing:</strong> {potassiumRec.timing}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-primary/5 border-primary">
        <CardHeader>
          <CardTitle className="text-lg">
            forest-green Fertilization Best Practices
          </CardTitle>
        </CardHeader>
        <CardContent>
          <ul className="space-y-2 text-sm">
            <li className="flex items-start gap-2">
              ...
              <span>
                Always apply fertilizers in split doses to prevent nutrient burn
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary mt-1">•</span>
              <span>
                Irrigate after fertilizer application to help nutrient
                absorption
              </span>
            </li>
            <li className="flex items-start gap-2">
            ...
            <li className="flex items-start gap-2">
              <span className="text-primary mt-1">•</span>
              <span>
                Conduct soil tests every 3-4 months for accurate nutrient
                management
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary mt-1">•</span>
              <span>
                Organic matter application improves nutrient retention in soil
              </span>
            </li>
          </ul>
        </CardContent>
